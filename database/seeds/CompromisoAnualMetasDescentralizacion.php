...
class CompromisoAnualMetasDescentralizacion extends Seeder
{
    ...
    public function run()
    {
        \DB::statement(" INSERT INTO compromiso_anual.metas_descentralizacion(id_provincia,primer_cuatrimestre,segundo_cuatrimestre,tercer_cuatrimestre,year)
	(
		SELECT *
		FROM dblink('dbname=sumardw host=192.6.0.118 user=postgres password=LatinoSandwich007',
		    'SELECT id_provincia,primer_cuatrimestre,segundo_cuatrimestre,tercer_cuatrimestre,year
			    FROM compromiso_anual.metas_descentralizacion')
