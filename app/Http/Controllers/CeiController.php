...
class CeiController extends Controller {
	 ...
	 *
	 * @param int $periodo Periodo de medicion
	 * @param int $linea   Linea de cuidado
	 *
	 * @return array
	 */
	...
	public function getIndicadorCei($periodo, $id_indicador) {
		...
		foreach ($provincias as $provincia) {
			...
			foreach ($beneficiarios as $key => $beneficiario) {
				...
				} elseif (isset($calculo->denominador->id)) {
			...
			foreach ($beneficiarios as $key => $beneficiario) {
				...
				foreach ($calculo->numerador->prestaciones as $prestacion) {
					...
					if (is_array($prestacion->codigos)) {
						$existe = DB::table('cei.mv_prestaciones_'.str_replace('-', '', $periodo))->where('clave_beneficiario', $beneficiario)
						                                                                          ->whereIn('codigo_prestacion', $prestacion->codigos)
							->whereBetween('fecha_prestacion', [$fechas['min_prestacion'], $fechas['max_prestacion']])
							->first();
					} else {
	...
	public function generarExcelLinea($periodo, $linea) {
		...
		Excel::create(
			'Linea de cuidado N°'.$linea.' '.$periodo,
			function ($e) use ($arrayCompletoInicial, $arrayCompletoMinima, $arrayCompletoAdecuada) {
				$e->sheet(
					'INICIAL',
					function ($s) use ($arrayCompletoInicial) {
						$s->setHeight(1, 20);
						$s->cells(
							'A1:'.$this->letraEnExcel($arrayCompletoInicial[1]).'1',
							function ($cells) {
						...
						$s->cells(
							'A1:'.$this->letraEnExcel($arrayCompletoInicial[1]).'25',
							function ($cells2) {
				...
				$e->sheet(
					'MINIMA',
					function ($s) use ($arrayCompletoMinima) {
						$s->setHeight(1, 20);
						$s->cells(
							'A1:'.$this->letraEnExcel($arrayCompletoMinima[1]).'1',
							function ($cells) {
								$cells->setBackground('#00004d');
								$cells->setFontColor('#ffffff');
								$cells->setFont(
						...
						$s->cells(
							'A1:'.$this->letraEnExcel($arrayCompletoMinima[1]).'25',
							function ($cells2) {
								$cells2->setAlignment('center');
								$cells2->setValignment('center');
								$cells2->setFontFamily('Arial');
				...
				if ($arrayCompletoAdecuada) {
					$e->sheet(
						'ADECUADA',
						function ($s) use ($arrayCompletoAdecuada) {
							$s->setHeight(1, 20);
							$s->cells(
								'A1:'.$this->letraEnExcel($arrayCompletoAdecuada[1]).'1',
								function ($cells) {
									$cells->setBackground('#00004d');
									$cells->setFontColor('#ffffff');
									$cells->setFont(
							...
							$s->cells(
								'A1:'.$this->letraEnExcel($arrayCompletoAdecuada[1]).'25',
								function ($cells2) {
									$cells2->setAlignment('center');
									$cells2->setValignment('center');
									$cells2->setFontFamily('Arial');
			...
			->store('xlsx', storage_path('exports/'.$periodo.'/'.$linea.' - '.Linea::find($linea)->nombre));
	...
	public function generarExcelLineaBeneficiario($periodo, $linea) {
		...
		Excel::create(
			'Linea de cuidado Desplegada N°'.$linea.' '.$periodo,
			function ($e) use ($arrayCompletoInicial, $arrayCompletoMinima, $arrayCompletoAdecuada) {
				$e->sheet(
					'INICIAL',
					function ($s) use ($arrayCompletoInicial) {
						...
						$s->cells(
							'A1:'.$this->letraEnExcel($arrayCompletoInicial[1]).$arrayCompletoInicial[2],
							function ($cells2) {
								$cells2->setAlignment('center');
								$cells2->setValignment('center');
								$cells2->setFontFamily('Arial');
				...
				$e->sheet(
					...
					function ($s) use ($arrayCompletoMinima) {
						...
						$s->cells(
							'A1:'.$this->letraEnExcel($arrayCompletoMinima[1]).$arrayCompletoMinima[2],
							function ($cells2) {
								$cells2->setAlignment('center');
								$cells2->setValignment('center');
								$cells2->setFontFamily('Arial');
				...
				if ($arrayCompletoAdecuada) {
					$e->sheet(
						...
						function ($s) use ($arrayCompletoAdecuada) {
							...
							$s->cells(
								'A1:'.$this->letraEnExcel($arrayCompletoAdecuada[1]).$arrayCompletoAdecuada[2],
								function ($cells2) {
									$cells2->setAlignment('center');
									$cells2->setValignment('center');
									$cells2->setFontFamily('Arial');
