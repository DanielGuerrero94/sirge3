...
class DdjjController extends Controller
{
    ...
    {
        $data = $this->getDataDDJJSirge($padron, $id);
        $pdf = PDF::loadView('pdf.ddjj.sirge', $data);
            return $pdf->download("ddjj-sirge-$id.pdf");
    }

    /**
     * Devuelve la data necesaria para la DDJJ Sirge elegida
     * @param int $id
     *
     * @return array
     */
    public function getDataDDJJSirge($padron, $id)
    {
        $resumen = [
            'in' => 0,
            'out' => 0,
        ...
        if ($padron == 4) {
            ...
            foreach ($lotes as $key => $lote) {
                $dr = ResumenLote::findOrFail($lote->lote);
                $resumen_dr['validos'] += $dr->validos;
                $resumen_dr['ausentes'] += $dr->ausentes;
                $resumen_dr['errores'] += $dr->errores;
                $lote->validos = $dr->validos;
                $lote->ausentes = $dr->ausentes;
                $lote->errores = $dr->errores;
		
            }

            $resumen = array_merge($resumen, $resumen_dr);
        } else {
            $lotes = Lote::join('ddjj.sirge', 'sistema.lotes.lote', '=', DB::raw('any(ddjj.sirge.lote)'))
            ->where('id_impresion', $id)
            ->select('sistema.lotes.lote', 'registros_in', 'registros_out', 'registros_mod')
