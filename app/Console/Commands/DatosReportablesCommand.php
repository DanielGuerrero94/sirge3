...
class DatosReportablesCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info("El lote {$lote} tiene {$count} prestaciones que requieren dato reportable.");
        foreach ($ids_prestaciones as $id) {
            ...
            if (!$resultado['success']) {
                ...
                if (array_key_exists('tiene', $resultado)) {
                    $validos = [
                        'ids' => $this->mapearMotivos($resultado['tiene'])
                    ];
                }
            ...
            $statement = "INSERT into logs.revision (id_prestacion";
            $statement .= ",lote";
            $statement .= !is_null($validos)?",validos":"";
            $statement .= !is_null($ausentes)?",ausentes":"";
            $statement .= !is_null($errores)?",errores":"";
            $statement .= ") values (";
            $statement .= $id_prestacion;
            $statement .= ",".$lote;
            $statement .= !is_null($validos)?",'".json_encode($validos)."'":"";
            $statement .= !is_null($ausentes)?",'".json_encode($ausentes)."'":"";
            $statement .= !is_null($errores)?",'".json_encode($errores)."'":"";
            $statement .= ")";
